...
def seed_vulnerabilities(db: Session, csv_path: Optional[str] = None) -> pd.DataFrame:
    ...
    for _, row in hvi_df.iterrows():
        ...
        vuln_obj = WardVulnerability(
            ...
            data_source_url=str(row.get("data_source_url", "https://censusindia.gov.in / MoSPI Periodic Labour Force Survey (PLFS)")),
            data_pulled_at=str(row.get("data_pulled_at", datetime.now(timezone.utc).strftime("%Y-%m-%d"))),
